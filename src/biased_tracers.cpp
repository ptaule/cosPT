...
{
    double q1 = std::sqrt(tables.comp_dot_products()
            .at(static_cast<size_t>(arguments[0]))
            .at(static_cast<size_t>(arguments[0])));
    double q2 = std::sqrt(tables.comp_dot_products()
            .at(static_cast<size_t>(arguments[1]))
            .at(static_cast<size_t>(arguments[1])));

    /* cosine(q1,q2) */
    double cos12 = tables.comp_dot_products()
            .at(static_cast<size_t>(arguments[0]))
            .at(static_cast<size_t>(arguments[1]))
            / (q1*q2);

    double mu1 = tables.comp_los_projection().at(static_cast<size_t>(arguments[0])) / q1;
    double mu2 = tables.comp_los_projection().at(static_cast<size_t>(arguments[1])) / q2;
        ...
        )
{
    ...
    double q2 = std::sqrt(tables.comp_dot_products()
            .at(static_cast<size_t>(arguments[1]))
            .at(static_cast<size_t>(arguments[1])));
    double q3 = std::sqrt(tables.comp_dot_products()
    ...
    double mu1 = tables.comp_los_projection().at(static_cast<size_t>(arguments[0])) / q1;
    double mu2 = tables.comp_los_projection().at(static_cast<size_t>(arguments[1])) / q2;
    double mu3 = tables.comp_los_projection().at(static_cast<size_t>(arguments[2])) / q3;
    ...
    if (q12_label != tables.loop_params.zero_label()) {
        ...
        double q12 = std::sqrt(tables.comp_dot_products()
                .at(static_cast<size_t>(q12_label))
                .at(static_cast<size_t>(q12_label)));

        /* cosine(q1+q2,q3) */
        double cos3_12 = tables.comp_dot_products()
                .at(static_cast<size_t>(q12_label))
                .at(static_cast<size_t>(arguments[2]))
                / (q1*q2);

        double mu12 = tables.comp_los_projection().at(static_cast<size_t>(q12_label)) / q12;
