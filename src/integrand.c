...
{
    table_pointers_t* const data_tables = input->data_tables;
        ...
        )
{
    ...
    for (int i = 0; i < pow(2,m-1); ++i) {
        ...
        vfloat k1 = compute_k1(diagram->m, rearrangement, signs,
                (const vfloat (*)[])input->data_tables->bare_scalar_products);
        int h_theta = heaviside_theta(diagram->m, k1, rearrangement,
                input->data_tables->Q_magnitudes);
        if (h_theta == 0) {
#if DEBUG >= 2
...
{
    // Alias pointer for convenience/readability
    table_pointers_t* const data_tables = input->data_tables;

    input->data_tables->Q_magnitudes = vars->magnitudes;

    // Initialize kernel tables and RHS tables
    initialize_tables(data_tables);
    ...
    for (int i = 0; i < N_DIAGRAMS; ++i) {
        vfloat diagram_result =
            loop_momenta_symmetrization(&(diagrams[i]), input);
