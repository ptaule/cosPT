    ...
    try {
        ...
        if (cfg.lookupValue("input_ps_rescale", r_str)) {
            if (r_str.compare("2pi^-3") == 0) {
                set("input_ps_rescale", pow(TWOPI,-3));
            }
        ...
        /* If not found as string, try double */
            ...
            set("input_ps_rescale", r_double);
        }
        /* Last, try integer */
        else if (cfg.lookupValue("input_ps_rescale", r_int)) {
            set("input_ps_rescale", r_int);
        }
    }
    catch (const libconfig::SettingTypeException& tex) {
        throw ConfigException(
    ...
    )
{
    try {
        ...
        if (cuba_cores == 0 && !set_param_value<int>(cuba_settings, "cores")) {
...
void Config::set_cuba_statefile(const libconfig::Setting& cuba_settings)
{
    try {
        ...
        if (set_param_value<string>(cuba_settings, "statefile")) {
